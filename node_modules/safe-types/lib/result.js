...
class Result {
    ...
    match(matcher) {
        switch (this.result.variant) {
            default:
                return utils_1.expect_never(this.result, "Invalid `Result` variant");
            case variant_1.ResultVariant.Ok:
                return matcher.Ok(this.result.value);
            case variant_1.ResultVariant.Err:
    ...
    is_ok() {
        return this.match({
            Ok: utils_1.always_true,
            Err: utils_1.always_false,
        });
    }
    /**
    ...
    ok() {
        return this.match({
            Ok: index_1.Option.Some,
            Err: index_1.Option.None,
        });
    }
    /**
    ...
    err() {
        return this.match({
            Ok: index_1.Option.None,
            Err: index_1.Option.Some,
        });
    }
    /**
    ...
    task() {
        return this.match({
            Ok: task_1.Task.of_ok,
            Err: task_1.Task.of_err,
        });
    }
    /**
     ...
     */
    tap(fn) {
        this.match({ Err: utils_1.noop, Ok: fn });
        return this;
    }
    /**
     ...
     */
    tap_err(fn) {
        this.match({ Err: fn, Ok: utils_1.noop });
        return this;
    }
    /**
     ...
     */
    static Ok(val) {
        return new Result(ok_1.Ok(val));
    }
    /**
     * Returns an Err result of the given type.
     */
    static Err(err) {
        return new Result(err_1.Err(err));
    }
    /**
